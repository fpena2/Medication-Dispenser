...
public func uploadData() {
    guard Bundle.main.url(forResource: "Schedule", withExtension: "json") != nil
else { return }
    let urlOfJson = documentDirectory.appendingPathComponent("Schedule.json")
    Amplify.Storage.uploadFile(key: "Schedule.json", local: urlOfJson,
...
func downloadData(imageName: String) {

    _ = Amplify.Storage.downloadFile(
            key: imageName,
            local: documentDirectory.appendingPathComponent(imageName),
            progressListener: { progress in
                print("Progress: \(progress)")
            }, resultListener: { event in
                switch event {
...
func downloadJson() {
    _ = Amplify.Storage.downloadFile(
            key: "aiOutput.json",
            local: documentDirectory.appendingPathComponent("aiOutput.json"),
            progressListener: { progress in
                print("Progress: \(progress)")
            }, resultListener: { event in
                switch event {
...
func downloadAioutputJson() {
    _ = Amplify.Storage.downloadFile(
            ...
            progressListener: { progress in
                ...
                switch event {
                case .success:
                    print("download aioutput Completed")
                case .failure(let storageError):
                    print("Failed: \(storageError.errorDescription). \(storageError.recoverySuggestion)")
                }
            })
...
func removeFile() {
    let fileNameToDelete = "aiOutput.json"
    var filePath = ""
